...
export default class Overlay {
  ...
  constructor(name, version) {
    ...
    this.templateManager = null; // The Template manager instance. Later populated when setTemplateManager is called
  ...
  /** Builds the layers panel UI.
   * @param {HTMLElement} parent - The parent element to attach the panel to.
   */
  buildLayersPanel(parent) {
    const templates = this.templateManager.templatesArray;

    const panel = document.createElement('div');
    panel.id = 'bm-layers-panel';
    ...
    list.id = 'bm-layers-list';
    panel.appendChild(list);

    templates.forEach((template, index) => {
      const item = document.createElement('div');
      item.className = 'bm-layer-item';
      item.dataset.id = template.storageKey;
      item.dataset.index = index;

      item.innerHTML = `
        <span class="bm-drag-handle">&#9776;</span>
        <span class="bm-layer-name">${template.displayName}</span>
        <div class="bm-layer-controls">
          <button class="bm-btn-vis ${template.enabled ? 'active' : ''}" title="Toggle Visibility">&#128065;</button>
          <button class="bm-btn-lock ${template.locked ? 'active' : ''}" title="Lock">&#128274;</button>
          <input type="range" class="bm-opacity-slider" min="0" max="1" step="0.1" value="${template.opacity ?? 1}" title="Opacity">
          <button class="bm-btn-dup" title="Duplicate">&#128461;</button>
          <button class="bm-btn-del" title="Delete">&#128465;</button>
        </div>
      `;

      // Event Listeners
      item.querySelector('.bm-btn-vis').addEventListener('click', () => this.templateManager.toggleVisibility(template.storageKey));
      item.querySelector('.bm-btn-lock').addEventListener('click', () => this.templateManager.lock(template.storageKey));
      item.querySelector('.bm-opacity-slider').addEventListener('input', (e) => this.templateManager.setOpacity(template.storageKey, e.target.value));
      item.querySelector('.bm-btn-dup').addEventListener('click', () => this.templateManager.duplicateTemplate(template.storageKey));
      item.querySelector('.bm-btn-del').addEventListener('click', () => this.templateManager.deleteTemplate(template.storageKey));
    ...
    if (typeof Sortable !== 'undefined') {
      new Sortable(list, {
        ...
        onEnd: (evt) => {
          ...
          this.templateManager.reorderTemplate(id, evt.newIndex);
          // Note: The UI will be out of sync until the next full rebuild.
          // For a more responsive feel, the panel should be redrawn here.
        }
      });
    }
  }
