...
overlayMain.setTemplateManager(templateManager); // Sets the template manager

// ---------- ASYNC INIT (no top-level await) ----------
(async () => {
  try {
    // cross-engine GM storage wrappers (TM/VM/GM4)
    const getVal = typeof GM?.getValue === 'function'
      ? GM.getValue
      : async (k, d) => (typeof GM_getValue === 'function' ? GM_getValue(k, d) : d);

    const setVal = typeof GM?.setValue === 'function'
      ? GM.setValue
      : async (k, v) => { if (typeof GM_setValue === 'function') return GM_setValue(k, v); };

    // init templates
    await templateManager.init(apiManager);

    // user settings
    let raw = await getVal('bmUserSettings', '{}');
    if (typeof raw === 'undefined' || raw === null) raw = '{}';
    const userSettings = (typeof raw === 'string') ? (safeJSON(raw) || {}) : (raw || {});
    console.log(userSettings);
    console.log(Object.keys(userSettings).length);

    if (!userSettings || Object.keys(userSettings).length === 0) {
      const uuid = (crypto?.randomUUID?.() || uuidv4Fallback());
      console.log(uuid);
      await setVal('bmUserSettings', JSON.stringify({ uuid }));
    }
    ...
    if ((userSettings?.telemetry == undefined) || (userSettings?.telemetry > 1)) { // retrigger notice
    ...
    const layersContainer = document.querySelector('#bm-layers-container');
    if (layersContainer) {
      overlayMain.buildLayersPanel(layersContainer);
    }
        ...
        (instance, img) => {
          ...
          img.addEventListener('click', () => {
            ...
            const elementsToToggle = [
              ...
              '#bm-layers-container',              // Layers panel container
