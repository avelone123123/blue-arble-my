...
export default class TemplateManager {
  ...
  constructor(name, version, overlay) {
    ...
    this.templatesArray = []; // Array of template class instances currently loaded (Template)
    this.templatesJSON = { templates: {} }; // All templates currently loaded (JSON)
    this.templatesShouldBeDrawn = true; // Should ALL templates be drawn to the canvas?
    this.tileProgress = new Map(); // Tracks per-tile progress stats {painted, required, wrong}
  }

  async init(apiManager) {
    await this.storageManager.init();
    const storedTemplates = await this.storageManager.getTemplates();
    if (storedTemplates && storedTemplates.length > 0) {
      const templatesData = {};
      storedTemplates.forEach(template => {
        templatesData[template.id] = template;
      });
      this.templatesJSON = { templates: templatesData };
      this.templatesArray = this.buildTemplatesFromJSON(this.templatesJSON.templates, apiManager);
    } else {
  ...
  async createTemplate(blob, name, coords) {
    ...
    await this.storageManager.putTemplate({ id: storageKey, ...this.templatesJSON.templates[storageKey] });
  ...
  /** Stores the JSON object of the loaded templates into IndexedDB storage.
   * @since 0.72.7
   */
  async #storeTemplates() {
    // This method is now primarily for batch updates, like after reordering.
    for (const template of this.templatesArray) {
      const templateData = this.templatesJSON.templates[template.storageKey];
      if (templateData) {
          await this.storageManager.putTemplate({ id: template.storageKey, ...templateData });
      }
    }
  }

  async #takeSnapshot(templateId) {
    const templateData = this.templatesJSON.templates[templateId];
    if (templateData) {
      await this.storageManager.putSnapshot(templateId, { data: templateData });
    }
  }

  /** Deletes a template from the JSON object.
   * Also delete's the corrosponding {@link Template} class instance
   * @param {string} id - The storage key of the template to delete.
   */
  async deleteTemplate(id) {
    if (!this.templatesJSON?.templates?.[id]) return;

    // Remove from JSON
    delete this.templatesJSON.templates[id];

    // Remove from array
    const index = this.templatesArray.findIndex(t => t.storageKey === id);
    if (index > -1) {
      this.templatesArray.splice(index, 1);
    }

    await this.storageManager.deleteTemplate(id);

    await this.#renumberSortIDs();
    await this.#storeTemplates(); // Re-saves the renumbered templates
    // TODO: Notify overlay to re-render layers panel
  }

  /** Toggles the visibility of a template.
   * @param {string} id - The storage key of the template.
   */
  async toggleVisibility(id) {
    if (!this.templatesJSON?.templates?.[id]) return;

    const templateJSON = this.templatesJSON.templates[id];
    templateJSON.enabled = !templateJSON.enabled;

    const template = this.templatesArray.find(t => t.storageKey === id);
    if (template) {
      template.enabled = templateJSON.enabled;
    }

    await this.storageManager.putTemplate({ id: id, ...this.templatesJSON.templates[id] });

    await this.#storeTemplates();
    // TODO: Trigger a redraw and update UI
  }

  /** Sets the opacity of a template.
   * @param {string} id - The storage key of the template.
   * @param {number} opacity - The opacity value (0.0 to 1.0).
   */
  async setOpacity(id, opacity) {
    if (!this.templatesJSON?.templates?.[id]) return;

    const value = Math.max(0, Math.min(1, opacity)); // Clamp value
    this.templatesJSON.templates[id].opacity = value;
  ...
  /** Toggles the lock state of a template.
   * @param {string} id - The storage key of the template.
   */
  async lock(id) {
    if (!this.templatesJSON?.templates?.[id]) return;
    ...
    const templateJSON = this.templatesJSON.templates[id];
    templateJSON.locked = !templateJSON.locked;

    const template = this.templatesArray.find(t => t.storageKey === id);
    if (template) {
      template.locked = templateJSON.locked;
    }

    await this.storageManager.putTemplate({ id: id, ...this.templatesJSON.templates[id] });

    await this.#storeTemplates();
    // TODO: Update UI to show lock state
  }

  /** Reorders a template to a new index.
   * @param {string} id - The storage key of the template to move.
   * @param {number} newIndex - The new index for the template.
   */
  /** Duplicates a template.
   * @param {string} id - The storage key of the template to duplicate.
   */
  async duplicateTemplate(id) {
    const originalTemplate = this.templatesArray.find(t => t.storageKey === id);
    if (!originalTemplate) return;

    // Deep copy the template data from JSON
    const originalTemplateJSON = JSON.parse(JSON.stringify(this.templatesJSON.templates[id]));

    const newTemplate = new Template({
      displayName: `${originalTemplate.displayName} (copy)`,
      sortID: this.templatesArray.length,
      authorID: originalTemplate.authorID,
      file: null, // File blob is not needed for duplication from existing data
      coords: originalTemplate.coords
    });

    // Copy properties from the original template instance
    newTemplate.chunked = originalTemplate.chunked;
    newTemplate.colorPalette = originalTemplate.colorPalette;
    newTemplate.pixelCount = originalTemplate.pixelCount;

    const storageKey = `${newTemplate.sortID} ${newTemplate.authorID}`;
    newTemplate.storageKey = storageKey;

    this.templatesJSON.templates[storageKey] = {
      ...originalTemplateJSON,
      name: newTemplate.displayName
    };

    await this.storageManager.putTemplate({ id: storageKey, ...this.templatesJSON.templates[storageKey] });

    this.templatesArray.push(newTemplate);

    // TODO: Notify overlay to re-render layers panel
  }

  async reorderTemplate(id, newIndex) {
    const template = this.templatesArray.find(t => t.storageKey === id);
    if (!template) return;

    const oldIndex = this.templatesArray.indexOf(template);
    if (oldIndex > -1) {
      this.templatesArray.splice(oldIndex, 1);
      this.templatesArray.splice(newIndex, 0, template);
    }

    this.#renumberSortIDs();
    await this.#storeTemplates();
    // TODO: Notify overlay to re-render layers panel
  }

  /** Renumbers the sortID for all templates to ensure they are contiguous.
   * This should be called after any add, remove, or reorder operation.
   */
  #renumberSortIDs() {
    this.templatesArray.forEach((template, index) => {
      const oldKey = template.storageKey;
      const newSortID = index;

      if (template.sortID === newSortID) return; // No change needed

      template.sortID = newSortID;
      const newKey = `${newSortID} ${template.authorID}`;
      template.storageKey = newKey;

      // Update the key in the JSON object
      if (this.templatesJSON.templates[oldKey]) {
        this.templatesJSON.templates[newKey] = this.templatesJSON.templates[oldKey];
        delete this.templatesJSON.templates[oldKey];
      }
    });
  }
